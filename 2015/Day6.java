...
public class Day6 {
    ...
    public Day6() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            boolean turnOn = lines[i].contains("turn");
            boolean turnState = false;
            int turnOnOffset = 0;

            if (turnOn) {
                turnState = split[1] == "on";
                turnOnOffset = 1;
            }

            String[] start = split[1 + turnOnOffset].split(",");
            int startX = Integer.parseInt(start[0]);
            int startY = Integer.parseInt(start[1]);

            String[] end = split[3 + turnOnOffset].split(",");
            int endX = Integer.parseInt(end[0]);
            int endY = Integer.parseInt(end[1]);

            for (int y = startY; y < endY; y++) {
                for (int x = startX; x < endX; x++) {
                    lightGrid[y][x] = turnOn ? turnState : !lightGrid[y][x];
                }
            }
        }
        ...
        for (int y = 0; y < lightGrid.length; y++) {
            for (int x = 0; x < lightGrid[y].length; x++) {
                onCount += lightGrid[y][x] == true ? 1 : 0;
            }
        }
